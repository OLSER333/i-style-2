...
class SalonCardModel extends PurpleModel
{
    function __construct(array $salon, string $view = 'components/salon_card', bool $isSalonPage = false)
    {
        ...
        $query = [
            "review.review_object_id = $salon_id",
            "review.review_object = 'seller'",
        ...
        if ($isSalonPage === true) {
            ...
            $arrTopup = [
                [new ReviewFormModel($variables['name'], 'leave_a_review_about_the_' . $variables['type'], 'describe_your_experience_at_the_' . $variables['type'], false), 'review-button-js', 'active'],
            ];

            if (!empty($reviews)) {
                $arrTopup[] = [new ReviewModel($review, 'show-review'), 'review-uncover-js', 'active'];
            }

            $variables['review_topup'] = new TopupModel(new ReviewFormModel($variables['name'], 'leave_a_review_about_the_' . $variables['type'], 'describe_your_experience_at_the_' . $variables['type'], false), 'review-button-js', topups: $arrTopup);
        }
        $this->data = $variables;
