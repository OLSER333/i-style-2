...
class CatalogServiceModel extends PurpleModel
{
    function __construct(array $service, bool $displaySubCategory = false, string $view = 'components/service_card', bool $isCatalogService = true, bool $isServicePage = false)
    {
        ...
        $arrTopup = [
            [new ReviewFormModel($variables['name'], 'leave_a_review_about_the_service', 'describe_your_experience_about_the_service', true), 'review-button-js', 'active'],
            [new ServiceRegistrationModel(), 'service-registration-js', 'active']
        ];
        ...
        if ($variables['reviews_count'] > 0) {

            if (!empty($reviews)) {

                array_push($arrTopup, [new ReviewModel($reviews[0], 'show-review'), 'review-uncover-js', 'active']);
            }
        }
        ...
        foreach ($reviews as $key => $review) {
            ...
            $reviewModelPhotos = $reviewModel->data['photos'];

            if (!empty($reviewModelPhotos)) {
                foreach ($reviewModelPhotos as $reviewPhoto) {
                    $variables['slider_photos'][] = $reviewPhoto['photo_path'];
                }
            }

            $reviews[$key] = $reviewModel;
        }

        $variables['reviews'] = $reviews;

        if ($isServicePage === true) {
            $variables['review_topup'] = new TopupModel(new ReviewFormModel($variables['name'], 'leave_a_review_about_the_service', 'describe_your_experience_about_the_service', true), 'review-button-js', topups: $arrTopup);
        }
