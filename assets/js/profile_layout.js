window.addEventListener('DOMContentLoaded', () => {
  ...
  if (document.querySelectorAll('.notice-mb-button').length == 0) {
    return;
  }
  ...
  window.addEventListener('resize', (e) => {
    if (document.documentElement.clientWidth < 1025) {
      ...
      if (contentTitle) {
        titleFrom.prepend(contentTitle);
      }
    }
    ...
    if (document.documentElement.clientWidth < 771) {
      ...
      if (noticesBox) {
        noticeAfterBlock.after(noticesBox);
      } 
      ...
      if (noticesBox) {
        blockBeforeNotice.after(noticesBox);
      } 
    }
