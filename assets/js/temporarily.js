...
$(document).ready(function () {
  ...
  if (document.querySelectorAll('.slider-photos-list').length == 0) {
    return;
  }

  let = sliderListX = sliderX.querySelector('.slider-photos-list'),
    ...
    nextTrfX = 0,
    prevTrfX = 0,
    lastTrfX = --slidesX.length * slideWidthX,
    ...
    swipeAction = function () {
      ...
      if (!isSwipeX && !isScrollX) {
        let posYX = Math.abs(posY2X);
        if (posYX > 7 || posX2X === 0) {
          isScrollX = true;
          allowSwipeX = false;
        } else if (posYX < 7) {
      ...
      if (isSwipeX) {
        ...
        if (slideIndexX === --slidesX.length) {
          console.log(slideIndexX, slidesX.length);
          if (posInitX > posX1X) {
